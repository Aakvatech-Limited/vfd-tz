...
def get_cert_serial(registration_doc):
	...
	cert_serial_hex_spaced = " ".join(a + b for a, b in zip(t, t, strict=False))
...
def get_latest_registration_doc(company, throw=True):
	...
	if not len(doc_list):
		if throw:
			frappe.throw(
				_("No active registration found for {0}. Please contact VFD Provider.").format(company)
			)
		return
	...
	if doc.is_blocked:
		...
		if throw:
			frappe.throw(
				_(f"VFD is BLOCKED for <b>{company}</b>.<br>Message from TRA:<br><b>{doc.tra_message}</b>")
			)
		else:
			frappe.msgprint(
				_(f"VFD is BLOCKED for <b>{company}</b>.<br>Message from TRA:<br><b>{doc.tra_message}</b>")
