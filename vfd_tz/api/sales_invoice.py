...
def vfd_validation(doc, method):
    if doc.base_net_total == 0:
        frappe.throw(
            _("Base net amount is zero. Correct the invoice and retry."))
...
def enqueue_posting_vfd_invoice(invoice_name):
    ...
    if not registration_doc.vfd_start_date:
        frappe.throw(_("VFD Start Date not set in VFD Registration. Please set it in VFD Registration"))
...
def posting_all_vfd_invoices():
    ...
    for company in company_list:
        ...
        if last_sent_success_gc + 1 != first_to_send_gc:
            frappe.throw(_("Invoice sequence out of order. Last GC Sent Successfully is {0}. First to send GC is {1}. Check failed VFD Invoice Posting Info").format(
                last_sent_success_gc, first_to_send_gc))
...
def get_item_taxcode(item_tax_template=None, item_code=None, invoice_name=None):
    if not item_tax_template:
        if item_code and invoice_name:
            frappe.throw(_("Item Taxes Template not set for item {0} in invoice {1}".format(
    ...
    if item_tax_template:
        ...
        if vfd_taxcode:
            ...
            frappe.throw(
                _("VFD Tax Code not setup in {0}".format(item_tax_template)))
    return taxcode
...
def get_payments(payments, base_total):
    ...
    for payment in payments:
        ...
        if vfd_pmttype:
            ...
            frappe.throw(
                _("VFD Payment Type in Mode of Payment not setup in {0}".format(payment.mode_of_payment)))
    ...
    if base_total > total_payments_amount:
        ...
        payments_dict.append(
            {"PMTAMOUNT": flt(base_total - total_payments_amount, 2)})
...
def before_update_after_submit(doc, method):
    ...
    if doc.vfd_status == "Success":
        frappe.throw(
            _("Cannot change Sales Invoice after VFD Status is Success!"))
