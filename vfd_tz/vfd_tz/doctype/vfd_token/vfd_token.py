...
from vfd_tz.api.utils import get_latest_registration_doc, to_base64
...
def get_token(company, force=False):
	...
	if len(token_list) and not force:
		...
		token_doc = frappe.get_doc(
			{
				...
				"expires_date": add_to_date(now(), seconds=(int(token_data.get("expires_in") or 0) - 1000)),
		...
		if token_doc.ackcode == "8":
			frappe.log_error(
				f"ACKCODE: {token_doc.ackcode} - ACKMSG: {token_doc.ackmsg}",
				"BLOCK ACKCODE found",
			)
			...
			frappe.log_error(f"{token_doc.ackcode} - {token_doc.ackmsg}", "ackcode normal")
		doc.tra_message = token_doc.ackmsg
		doc.save(ignore_permissions=True)
		frappe.db.commit()
		if token_doc.ackcode in ["8", "18"]:
			frappe.log_error(
				_("TRA has rejected the token request with code {0} and message {1}").format(
					token_doc.ackcode, token_doc.ackmsg
				),
				"VFD Command Found",
			)
