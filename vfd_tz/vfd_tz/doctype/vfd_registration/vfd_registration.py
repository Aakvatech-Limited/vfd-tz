...
from vfd_tz.api.utils import get_cert_serial, get_signature, to_base64
...
def get_registration(doc):
	data = "<REGDATA><TIN>{}</TIN><CERTKEY>{}</CERTKEY></REGDATA>".format(
		doc.tin, doc.get_password("certkey")
	)
	signenature = get_signature(data, doc)
	extend_data = f'<?xml version="1.0" encoding="UTF-8"?><EFDMS>{data}<EFDMSSIGNATURE>{signenature}</EFDMSSIGNATURE></EFDMS>'
	...
	response = requests.request("POST", url, headers=headers, data=extend_data, timeout=60)
