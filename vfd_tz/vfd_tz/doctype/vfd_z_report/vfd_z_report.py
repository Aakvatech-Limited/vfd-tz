...
class VFDZReport(Document):
	...
	def set_data(self):
		...
		if len(invoices) > 0:
			...
			self.dailytotalamount = get_gross_between(company, self.serial, self.vfd_gc_from, self.vfd_gc_to)
		else:
		...
		self.ticketsvoidtotal = sum(
			invoice["base_rounded_total"] or invoice["base_grand_total"] for invoice in canceled_invoices
		)
		self.set_payments()
	...
	def get_canceled_invoices(self):
		...
		if not report_start_date:
			frappe.throw(
				_("VFD Z-Report Start Date is not set in VFD Registration {0}").format(self.vfd_registration)
			)
	...
	def update_canceled_invoices(self):
		...
		for invoice in canceled_invoices:
			if not invoice.vfd_z_report:
				frappe.db.set_value("Sales Invoice", invoice.name, "vfd_z_report", self.name)
...
def get_vattotals(items, vrn):
	...
	if items[0].parenttype == "VFD Tax Invoice":
		...
		for key in taxes_map:
			vattotals.setdefault(key, {"NETTAMOUNT": 0, "TAXAMOUNT": 0})
		for item in items:
			item_taxcode = get_item_taxcode(item.item_tax_template, item.item_code, item.parent)
...
def get_gross_between(company, serial, start, end):
    ...
    WHERE
        company = '{company}'
        and docstatus = 1
        and vfd_serial = '{serial}'
        and vfd_gc BETWEEN {start or 0} AND {end or 0}
    """,
		as_dict=True,
	)
...
def zreport_posting(doc):
	...
	zreport = {
		...
		"HEADER": [
			...
			{"LINE": (registration_doc.city or " ") + ", " + (registration_doc.country or " ")},
		],
	...
	zreport_xml = str(dict_to_xml(zreport, "ZREPORT")[39:]).replace("<None>", "").replace("</None>", "")
...
def make_vfd_z_report():
	...
	for vfd_registration in vfd_registration_list:
		try:
			last_z_report = frappe.get_last_doc("VFD Z Report", filters={"serial": vfd_registration.serial})
