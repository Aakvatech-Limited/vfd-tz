	...
	def before_submit(self):
		...
		for item in self.items:
			if not item.item_name:
				frappe.throw(_(f"Item Name not set for item {item.item_name}"))
			item_taxcode = item.item_taxcode

			if item_taxcode == 1 and registration_doc.vrn == "NOT REGISTERED":
				frappe.throw(
					_(
						f"Taxes SHOULD not set to 18pct for Standard Rate item {item.item_name} for NON VAT company"
					)
				)

	def after_insert(self):
		if self.docstatus == 1 and self.is_auto_generate_vfd:
			enqueue_posting_vfd_invoice(self.name)
			self.reload()

	def on_submit(self):
		if self.is_auto_generate_vfd:
			enqueue_posting_vfd_invoice(self.name)
			frappe.msgprint(_(f"Auto generated VFD for invoice {self.name}"), alert=True)
...
def enqueue_posting_vfd_invoice(invoice_name):
	...
	if doc.creation < registration_doc.vfd_start_date:
		frappe.throw(
			_("VFD Tax Invoice creation older than VFD Registration date! Cannot submit VFD to TRA.")
		)
	if doc.posting_date < registration_doc.vfd_start_date.date():
		frappe.throw(
			_("VFD Tax Invoice posting date older than VFD Registration date! Cannot submit VFD to TRA.")
		)
...
def posting_vfd_invoice(invoice_name):
	...
	for item in doc.items:
		item_data = {
			"ID": remove_special_characters(item.item_group if use_item_group else item.item_name),
